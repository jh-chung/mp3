...
```{r (Mapping a few things)/(Do we need this code?) , eval = FALSE}
...
leaflet(data = trail_cont) %>%
  ...
  addLegend("bottomleft", pal = qpal, 
            values = ~level, 
            title = "Difficulty (NPS Difficulty, All Easy)") %>%
  addLayersControl(
...
qpal <- colorFactor(c("green", "yellow", "red"), 
                     domain = trail_cont$level) # Assignig a color factor to the difficulty of each trail
...
leaflet(data = trail_cont) %>%
  ...
  addPolygons(data = macleish_layers[["contours_3m"]] , weight = 1 , fillOpacity = 0.01 , group = "Contours (3 m)" , color = "lightgrey") %>%

  addPolygons(data = macleish_layers[["boundary"]], weight = 1, fillOpacity = 0.1, group = "Boundaries" , color = "purple" , label = "Boundaries") %>%

  addMarkers(data = macleish_layers[["challenge_courses"]],  group = "Challenge Courses" , popup = ~cmt) %>%

  addPolylines(data = macleish_layers[["streams"]], weight = 1, group = "Streams") %>%
  addPolygons(data = macleish_layers[["forests"]], weight = 1, group = "Forests" , color = "green" , label = ~Sheet1__Na) %>%
  addPolygons(data = macleish_layers[["research"]], weight = 1, group = "Research Plots" , color = "brown" ,label = ~OBJECTID) %>%
  addPolygons(data = macleish_layers[["buildings"]], 
              weight = 1, popup = ~name,  color = "grey" , group = "Buildings") %>%
  addMarkers(data = macleish_layers[["landmarks"]], 
             popup = ~Label, group = "Landmarks") %>%
  
  
    addLayersControl(
    baseGroups = c("OpenStreetMap", "Topography", "Satellite", "Toner Lite"),
    overlayGroups = c("All Trails" , "Easy Trails" , "Medium Trails" , "Hard Trails" , "Boundaries" ,"Streams" , "Forests" , "Research Plots" , "Buildings" , "Landmarks", "Challenge Courses" , "Contours (3 m)") ,
    options = layersControlOptions(collapsed = TRUE)
  ) 
```

```{r Finding the number of Intersections per Trail, message = FALSE}
