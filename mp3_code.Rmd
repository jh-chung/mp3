...
trail_cont <- st_join(trails, contour_data, join = st_within ) %>% # st_withing is the join type
  select(total_length , name , change , geometry) %>% # We only need these variables
  mutate(hard_meas = sqrt(2*(contour_data$change*0.000621371)*(trails$total_length*0.000621371))) %>% # Using the equation from the National Park Service
  arrange((hard_meas)) # Want to arrange it in desc(hard_meas)
```
