...
```{r}
trail_cont <- st_join(trails, macleish_layers[["contours_3m"]], join = st_within ) %>% 
  select(total_length , name , geometry)
  mutate(hard_meas = sqrt(2*(contour_data$change*0.000621371)*(trails$total_length*0.000621371))) %>%
  arrange(desc(hard_meas))
```
